            ...
            container.Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    ...
                    row.RelativeItem().PaddingLeft(10).Column(col =>
                    {
                        ...
                        col.Item().Text(empresa.Direccion ?? "Direcci贸n").FontSize(9);
                ...
                column.Item().Row(row =>
                {
                    ...
                    row.RelativeItem().Text($"N煤mero: {venta.NumeroDocumento}").FontSize(10);
        ...
        private void ComposeContenido(IContainer container, Venta venta)
        {
            container.Column(column =>
            {
                ...
                column.Item().Table(table =>
                {
                    ...
                    {
                        columns.ConstantColumn(60);   // C贸digo
                        columns.ConstantColumn(50);   // Cantidad
                        columns.RelativeColumn();     // Descripci贸n
                    ...
                    table.Header(header =>
                    {
                        header.Cell().Element(CellStyle).Text("C贸digo").Bold();
                        header.Cell().Element(CellStyle).Text("Cant.").Bold();
                        header.Cell().Element(CellStyle).Text("Descripci贸n").Bold();
                ...
                column.Item().AlignRight().Column(col =>
                {
                    ...
                    if (venta.PorcentajeInteres > 0 || venta.MontoInteres > 0)
                    {
                        ...
                        col.Item().Row(r =>
                        {
                            r.ConstantItem(150).Text($"Inter閟 ({porcentaje:N1}%):").FontSize(11);
